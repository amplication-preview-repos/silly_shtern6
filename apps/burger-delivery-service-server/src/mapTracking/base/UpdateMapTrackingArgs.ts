/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MapTrackingWhereUniqueInput } from "./MapTrackingWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { MapTrackingUpdateInput } from "./MapTrackingUpdateInput";

@ArgsType()
class UpdateMapTrackingArgs {
  @ApiProperty({
    required: true,
    type: () => MapTrackingWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => MapTrackingWhereUniqueInput)
  @Field(() => MapTrackingWhereUniqueInput, { nullable: false })
  where!: MapTrackingWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => MapTrackingUpdateInput,
  })
  @ValidateNested()
  @Type(() => MapTrackingUpdateInput)
  @Field(() => MapTrackingUpdateInput, { nullable: false })
  data!: MapTrackingUpdateInput;
}

export { UpdateMapTrackingArgs as UpdateMapTrackingArgs };
